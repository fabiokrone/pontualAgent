...
ALTER TABLE ponto.usuarios
ADD COLUMN reset_token VARCHAR(100) NULL,
ADD COLUMN reset_token_expires_at TIMESTAMP NULL;
-- Opcional: Adicionar um índice ao token para otimizar buscas
CREATE INDEX ix_ponto_usuarios_reset_token ON ponto.usuarios (reset_token);


-- Tabela para registros de auditoria
CREATE TABLE IF NOT EXISTS ponto.auditoria (
    id SERIAL PRIMARY KEY,
    usuario_id INTEGER REFERENCES ponto.usuarios(id) ON DELETE SET NULL,
    username VARCHAR(50),
...
CREATE INDEX IF NOT EXISTS ix_usuarios_username ON ponto.usuarios (username);
CREATE INDEX IF NOT EXISTS ix_usuarios_email ON ponto.usuarios (email);

-- Adicionar a coluna CPF à tabela Servidores (conforme modelo)
ALTER TABLE ponto.servidores
ADD COLUMN IF NOT EXISTS cpf VARCHAR(11) UNIQUE NOT NULL;
CREATE INDEX IF NOT EXISTS ix_servidores_cpf ON ponto.servidores (cpf);

-- Remover a coluna Ambito da tabela Feriados (não presente no modelo)
-- ALTER TABLE ponto.feriados
-- DROP COLUMN IF EXISTS ambito;
-- Comentado: Decida se quer manter ou remover com base na sua necessidade.
-- Se remover, ajuste também o CHECK constraint.

-- Adicionar a chave estrangeira para justificativa_id em Batidas Processadas (conforme modelo)
ALTER TABLE ponto.batidas_processadas
DROP CONSTRAINT IF EXISTS fk_batidas_processadas_justificativa_id; -- Remove se já existir com outro nome

ALTER TABLE ponto.batidas_processadas
ADD CONSTRAINT fk_batidas_processadas_justificativa_id
FOREIGN KEY (justificativa_id) REFERENCES ponto.justificativas(id) ON DELETE SET NULL;

-- Aplicar o trigger de updated_at na nova tabela usuarios
DROP TRIGGER IF EXISTS update_usuarios_modtime ON ponto.usuarios;
CREATE TRIGGER update_usuarios_modtime BEFORE UPDATE ON ponto.usuarios
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();


-- Índices para a tabela de auditoria
CREATE INDEX IF NOT EXISTS idx_auditoria_usuario ON ponto.auditoria(usuario_id);
CREATE INDEX IF NOT EXISTS idx_auditoria_tabela ON ponto.auditoria(tabela);
CREATE INDEX IF NOT EXISTS idx_auditoria_acao ON ponto.auditoria(acao);
CREATE INDEX IF NOT EXISTS idx_auditoria_data ON ponto.auditoria(data_hora);
...
CREATE TRIGGER update_auditoria_modtime BEFORE UPDATE ON ponto.auditoria
    FOR EACH ROW EXECUTE FUNCTION ponto.update_updated_at_column();
